...
1. **Register the explicit App ID** — [Certificates, Identifiers & Profiles → Identifiers](https://developer.apple.com/account/resources/identifiers) → **+** → App IDs → App, Bundle ID **Explicit** = `ai.multica.mobile`. This has to come first because the New App form in step 2 only offers Bundle IDs that already exist ([register an App ID](https://developer.apple.com/help/account/identifiers/register-an-app-id)). Leave **Capabilities** alone — the app ships an empty entitlements file, so none are required. (In-App Purchase is on by default and can't be unchecked; that's harmless.) Anything that later adds an entitlement — push notifications, Associated Domains for universal links, Sign in with Apple, App Groups, iCloud — needs the matching capability enabled here *and* in the Xcode target, and capabilities can be changed after registration.
